    ...
    public Result<BuyerUser> login(@RequestBody LoginParam param) throws CoBusinessException{
        ...
        if(IntegerEnum.REGIST.getCode().equals(param.getType())){
            ...
            if(personal!=null){
                ...
                //存储token到redis
                userRedisService.saveUser(cereBuyerUser.getToken(), cereBuyerUser.getBuyerUserId());
            }
        }else {
            //登录
            if(personal!=null){
                ...
                personal.setToken(RandomStringUtil.getRandomCode(32, 0));
                //存储token到redis
                userRedisService.saveUser(personal.getToken(), personal.getBuyerUserId());
            }else {
