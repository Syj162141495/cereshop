...
public class CereShopCommentServiceImpl implements CereShopCommentService {
    ...
    public void like(LikeParam param, CereBuyerUser user) throws CoBusinessException {
        //查询之前是否点赞过
        CereBuyerCommentLike like=cereBuyerCommentLikeService.findByUserIdAndCommentId(user.getBuyerUserId(),param.getCommentId());
        if(like==null){
            //新增点赞数据
            like=new CereBuyerCommentLike();
            like.setBuyerUserId(user.getBuyerUserId());
            like.setCommentId(param.getCommentId());
            like.setIfLike(param.getIfLike());
            cereBuyerCommentLikeService.insert(like);
        }else {
            //更新点赞数据
            like.setIfLike(param.getIfLike());
            cereBuyerCommentLikeService.update(like);
        }
    }

    @Override
    public void update(CereShopComment cereShopComment) throws CoBusinessException {
        cereShopCommentDAO.update(cereShopComment);
    }
