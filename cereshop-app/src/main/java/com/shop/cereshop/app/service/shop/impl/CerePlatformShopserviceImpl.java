...
public class CerePlatformShopserviceImpl implements CerePlatformShopservice {
            ...
            list.forEach(shop -> {
                ...
                if (CollectionUtils.isNotEmpty(skus)) {
                    for (CartSku sku:skus) {
                        ProductBo productBo = cereShopProductService.fetchProductCache(sku.getShopId(), sku.getProductId(), sku.getSkuId(), user, false);
                        if (productBo != null) {
                            sku.setActivityType(productBo.getActivityType());
                            sku.setPrice(productBo.getPrice());
                        }
                    }
                }
                shop.setSkus(skus);
    ...
    private String genPoster(int env, Integer type, Long shopId, String shareHeadUrl, String shareName,
        ...
        if (env == 1 || env == 2) {
            ...
            if (type != null && type == 2) {
                url = "pages_category_page1/distributionModule/recruit?shopId=" + shopId;
            }
